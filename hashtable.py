...
class HashTable(object):
    ...
    def keys(self):
        """Return a list of all keys in this hash table.
            Time: O(b + i) - b is buckets and i is items
            Running time: O(n) - n = number of entries
        """
    ...
    def get(self, key):
        ...
        # Find the bucket the given key belongs in
        index = self._bucket_index(key) # O(n) - hash func is on
        bucket = self.buckets[index] # O(1)
    ...
    def _resize(self, new_size=None):
        ...
        Best and worst case space usage: O(akv * nb) = O(2n) = O(n) """
        ...
        for key, value in all_kv_entries:
            self.set(key, value)
